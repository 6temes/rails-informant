ENV["RAILS_ENV"] = "test"

require_relative "dummy/config/environment"
require "rails/test_help"
require "mocha/minitest"
require "resolv"
require_relative "support/error_group_helpers"
require_relative "support/stub_http"

# Create tables directly since we're not using standard migrations
ActiveRecord::Schema.define do
  unless ActiveRecord::Base.connection.table_exists?(:informant_error_groups)
    create_table :informant_error_groups do |t|
      t.string :fingerprint, null: false, index: { unique: true }
      t.string :error_class, null: false
      t.text :message
      t.string :severity, default: "error"
      t.string :status, default: "unresolved", null: false
      t.string :first_backtrace_line
      t.string :controller_action
      t.string :job_class
      t.integer :total_occurrences, default: 0, null: false
      t.references :duplicate_of, foreign_key: { to_table: :informant_error_groups }
      t.string :fix_sha
      t.string :original_sha
      t.string :fix_pr_url
      t.text :notes
      t.datetime :first_seen_at, null: false
      t.datetime :last_seen_at, null: false
      t.datetime :resolved_at
      t.datetime :fix_deployed_at
      t.datetime :last_notified_at
      t.datetime :last_occurrence_stored_at
      t.timestamps

      t.index [ :status, :last_seen_at ]
      t.index [ :status, :original_sha ]
      t.index [ :status, :resolved_at ]
      t.index [ :status, :total_occurrences ]
      t.index [ :status, :updated_at ]
      t.index [ :error_class ]
    end

    add_check_constraint :informant_error_groups,
      "duplicate_of_id IS NULL OR duplicate_of_id != id",
      name: "check_no_self_duplicate"
  end

  unless ActiveRecord::Base.connection.table_exists?(:informant_occurrences)
    create_table :informant_occurrences do |t|
      t.references :error_group, null: false,
        foreign_key: { to_table: :informant_error_groups }
      # Uses t.json instead of t.jsonb because tests run on SQLite which
      # lacks jsonb support. The canonical migration uses t.jsonb for PostgreSQL.
      t.json :backtrace
      t.json :exception_chain
      t.json :request_context
      t.json :user_context
      t.json :custom_context
      t.json :environment_context
      t.json :breadcrumbs
      t.string :git_sha
      t.timestamps

      t.index [ :error_group_id, :created_at ]
      t.index :created_at
    end
  end
end

class ActiveSupport::TestCase
  include ErrorGroupHelpers

  self.use_transactional_tests = true

  setup do
    RailsInformant.config = RailsInformant::Configuration.new
    RailsInformant.config.capture_errors = true
    RailsInformant.config.api_token = "test-token-00112233445566778899aabb"
    RailsInformant.reset_caches!
    RailsInformant::ContextFilter.reset!
    RailsInformant::Notifiers::CircuitBreaker.reset!
    Resolv.stubs(:getaddresses).returns([ "93.184.216.34" ])
  end

  teardown do
    RailsInformant::Current.reset
  end
end
