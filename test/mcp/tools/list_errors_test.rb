    ...
    module Tools
      class ListErrorsTest < Minitest::Test
        ...
        def test_returns_error_list
          ...
          @client.stubs(:list_errors).returns(paginated(errors))
        ...
        def test_passes_filters_to_client
          @client.expects(:list_errors).with(status: "resolved", q: "timeout").returns(paginated([]))

          ListErrors.call(server_context: @server_context, status: "resolved", q: "timeout")
        end

        def test_passes_duplicate_status_to_client
          @client.expects(:list_errors).with(status: "duplicate").returns(paginated([]))

          ListErrors.call(server_context: @server_context, status: "duplicate")
        end

        def test_uses_specified_environment
          staging_client = Client.new(url: "https://staging.example.com", token: "staging-token")
          staging_client.stubs(:list_errors).returns(paginated([]))
